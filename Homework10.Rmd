...
```{r}
personSubset = personData %>% filter(PER_TYP ==1)
```
...
tableJoin = left_join(personSubset, accidentData, by = c("STATE", "ST_CASE"))
```


Tally the number of accidents by day of the week (DAY_WEEK), hour of the day (HOUR) and gender (SEX). Visualize the results!
```{r}
tableJoin %>% filter(SEX == c(1,2)) %>% ggplot(aes(x = DAY_WEEK, fill = as.factor(HOUR.x))) + geom_bar() + facet_grid(~SEX) + scale_x_discrete(limits = c("1", "2", "3", "4", "5", "6", "7")) + xlab("Day of Week") + ylab("Count") + guides(fill=guide_legend("Hour"))

tableJoin %>% count(DAY_WEEK)
tableJoin %>% count(HOUR.x)
tableJoin %>% count(SEX)
head(tableJoin)
```
